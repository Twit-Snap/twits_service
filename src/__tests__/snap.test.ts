...
describe("Snap API Tests", () => {
	...
	describe("GET /snaps", () => {
		...
		it("should return all snaps when multiple snaps exist", async () => {
			...
			expect(
				response.body.data.map((snap: SnapResponse) => snap.content)
			).toEqual(["Test snap 3", "Test snap 2", "Test snap 1"]);
		});
		...
		it("should return all items if has param is undefined", async () => {
			...
			expect(
				response.body.data.map((snap: SnapResponse) => snap.content)
			).toEqual(["Test snap 3", "hello Test snap 2", "Test snap 1"]);
		});
		...
		it("should return items in unicode when 'has' is a emoji", async () => {
			...
			expect(
				response.body.data.map((snap: SnapResponse) => snap.content)
			).toEqual(["Test snap 3 😎🤓👆", "Test snap 1 😎😎😎"]);
		});
		...
		it("should return an array of snaps of the users it follows", async () => {
			...
			server.resetHandlers(
				...[
					http.get(
						`${process.env.USERS_SERVICE_URL}/users/${user.username}/followers`,
						() => {
			...
			expect(response.body.data).toHaveLength(2);
			expect(
				response.body.data.map((snap: SnapResponse) => snap.user.userId)
			).toEqual([4, 2]);
			expect(
				response.body.data.map((snap: SnapResponse) => snap.content)
			).toEqual(["Test snap 4", "Test snap 2"]);
		});
		...
		it("should return an array of snaps with the first one equal to the position of the offset", async () => {
			...
			expect(
				response.body.data.map((snap: SnapResponse) => snap.content)
			).toEqual(["Test snap 2", "Test snap 1"]);
		});
		...
		it("should return likes count if the user is public", async () => {
			...
			server.resetHandlers(
				...[
					http.get(
						`${process.env.USERS_SERVICE_URL}/users/testuser1`,
						() => {
							return HttpResponse.json({
								data: {
									name: "Test User 1",
					...
					http.get(
						`${process.env.USERS_SERVICE_URL}/users/testuser2`,
						() => {
							return HttpResponse.json({
								data: {
									name: "Test User 2",
			...
			expect(
				response.body.data.map((snap: SnapResponse) => snap.content)
			).toEqual(["Hello! Doing a #Test 2", "Hello! Doing a #Test 1"]);
			expect(
				response.body.data.map((snap: SnapResponse) => snap.likesCount)
			).toEqual([0, 0]);
		});
		...
		it("should return likes count if the user ask for themselves when it is private or not", async () => {
			...
			server.resetHandlers(
				...[
					http.get(
						`${process.env.USERS_SERVICE_URL}/users/${user.username}`,
						() => {
							return HttpResponse.json({
								data: {
									name: user.name,
	...
	describe("GET /snaps/:id", () => {
		...
		it("should return entities if withEntities is true", async () => {
			...
			expect(response.status).toBe(200);
			expect(response.body.data.entities.hashtags[0].text).toEqual(
				"#Test"
			);
		});
